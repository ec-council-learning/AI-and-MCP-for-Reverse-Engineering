...
@mcp.tool()
def read_str_from_memory(addr, size=32, encoding: str = "ascii") -> dict:
    """
    Reads memory bytes starting at the given address and decodes them into text.  
      - If the decoded string overflows into additional phrases or sentences, only the
        first complete phrase/sentence is kept and the rest is discarded.
      - If the decoded string ends with a **partial phrase or sentence**, the tool should
        be called again with an extended size until a full phrase/sentence is completed.
...
if __name__ == "__main__":
    ...
    mcp.run(
        ...
        port=9628,        # override default 8000
        path="/mcp/"       # the endpoint path clients must call
    )